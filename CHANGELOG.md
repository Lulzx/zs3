...
First release. Distributed mode becomes a real replicated cluster: writes
leave the node that receives them, reads fall back to peers, and the mesh
repairs itself after restarts and partitions.
...
- **Background replication worker.** Blob replication, DHT announces, and
  ...
  write latency no longer pays for cross-node copies. The ~1KB metadata push
  stays synchronous.
...
- **Clean 4xx errors for filesystem-size limits.** Keys that can't fit a
  filesystem (`NAME_MAX` per component, `PATH_MAX` total) now return
...
- **431 for oversized headers, 400 for oversized bodies.** Request headers
  over 8KB get a clean `431`; bodies over 5GB get `400` — no more silent
  truncation or connection close.
...
- **Hung peers can no longer wedge the event loop.** Peer sockets get 5s
  send/receive timeouts, preventing a deadlock between two nodes mid-request.
- **Unauthenticated `/_zs3/` endpoints now validate bucket/key names.**
- **`HeadObject` for directory prefixes.** Directory markers now answer HEAD
  instead of a spurious miss.
...
- New `test_replication.py`: a four-node suite (113 checks) covering
  ...
  routing-table wipe, and peer protocol validation, plus unit tests for the
  meta-entry parser.
- Distributed boto3 suite goes 71 → 72 passing.
